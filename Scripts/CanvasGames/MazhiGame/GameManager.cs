...
public class GameManager : MonoBehaviour
{
    ...
    private void Start()
    {
        GameObject gameObject = GameObject.Find("Camera_GameUI");
    ...
    public void CreatCard()
    {
        ...
        for (int i = 0; i < row; i++)//创建四行四列的卡片
        {
            for (int j = 0; j < col; j++)
            {
                ...
                cloneCard.transform.position = transform.position + offset + Vector3.up * i * (_cardH + _spaceY) + Vector3.right * j * (_cardH + _spaceX) - Vector3.forward * 0.3f;//
    ...
    void Update()
    {
        if (_gameover)//如果游戏结束则return
        {
            ...
            var objectsWithScript = Resources.FindObjectsOfTypeAll<SeeDoorManDialogueController>();
            foreach (var see in objectsWithScript)
            {
                see.isTalk = false;
            }
            //return;
    ...
    private void MouseInput()//鼠标点击
    {
        ...
        {
            print("鼠标点击");
            if (_currentTarget != null && _rotateCardList.Count < 2 && !_rotateCardList.Contains(_currentTarget))// && _rotateCardList.Count < 2 && !_rotateCardList.Contains(_currentTarget)
    ...
    public void CompareCards(Card card)//比较卡片---------------
    {
        ...
        if (_compareCardList.Count != 2) return;
    ...
    {
        Ray mouseRay = camera.ScreenPointToRay(Input.mousePosition);//从摄像机发出一条射线 (Ray) 到屏幕上的鼠标位置
        RaycastHit hitInfo;
